...
use crate::ast::*;
use crate::error::AgentQLError;

// ── Pest plumbing ─────────────────────────────────────────────

#[derive(Parser)]
#[grammar = "agentql.pest"]
pub struct AgentQLParser;

// ── Entry Point ───────────────────────────────────────────────

pub fn parse(source: &str) -> Result<Program, AgentQLError> {
    let pairs = AgentQLParser::parse(Rule::program, source)
        .map_err(|e| AgentQLError::ParseError(e.to_string()))?;

    let program_pair = pairs
        .into_iter()
        .next()
        .ok_or_else(|| AgentQLError::ParseError("empty program".into()))?;
...
fn parse_program(pair: Pair<Rule>) -> Result<Program, AgentQLError> {
    let mut imports = vec![];
        ...
        match inner.as_rule() {
            ...
            Rule::definition  => {
                ...
                match def.as_rule() {
                    ...
                    r => return Err(AgentQLError::UnexpectedRule(format!("{r:?}"))),
                }
            }
            Rule::EOI => {}
            r => return Err(AgentQLError::UnexpectedRule(format!("{r:?}"))),
...
fn parse_import(pair: Pair<Rule>) -> Result<ImportDecl, AgentQLError> {
...
fn parse_schema(pair: Pair<Rule>) -> Result<SchemaDef, AgentQLError> {
    let types = pair
        .into_inner()
        .map(parse_type_def)
        .collect::<Result<_, _>>()?;
...
fn parse_type_def(pair: Pair<Rule>) -> Result<TypeDef, AgentQLError> {
    let mut inner = pair.into_inner();
...
fn parse_field_def(pair: Pair<Rule>) -> Result<FieldDef, AgentQLError> {
    let mut inner = pair.into_inner();
    let name     = inner.next().unwrap().as_str().to_string();
...
fn parse_type_ref(pair: Pair<Rule>) -> Result<TypeRef, AgentQLError> {
    ...
    match inner_pair.as_rule() {
        Rule::scalar_type => {
            let kind = match inner_pair.as_str() {
                ...
                s => return Err(AgentQLError::ParseError(format!("unknown scalar: {s}"))),
            };
        ...
        r => Err(AgentQLError::UnexpectedRule(format!("{r:?}"))),
    }
}

// ── Module Proposals ──────────────────────────────────────────

fn parse_module_proposal(pair: Pair<Rule>) -> Result<ModuleProposal, AgentQLError> {
    let mut inner = pair.into_inner();
    let name = inner.next().unwrap().as_str().to_string();
...
fn parse_op_def(pair: Pair<Rule>) -> Result<OpDef, AgentQLError> {
    let mut inner = pair.into_inner();
    ...
    Ok(OpDef {
        ...
        name,
        args,
        returns: returns.ok_or_else(|| AgentQLError::ParseError("op_def missing return type".into()))?,
    })
}

// ── Queries, Mutations, Fragments, Subscriptions ──────────────

fn parse_query(pair: Pair<Rule>) -> Result<QueryDef, AgentQLError> {
    let mut inner = pair.into_inner();
    let name      = inner.next().unwrap().as_str().to_string();
...
fn parse_mutation(pair: Pair<Rule>) -> Result<MutationDef, AgentQLError> {
    let mut inner = pair.into_inner();
    let name      = inner.next().unwrap().as_str().to_string();
    let (variables, directives, pipelines) = parse_operation_body(inner)?;
...
fn parse_fragment(pair: Pair<Rule>) -> Result<FragmentDef, AgentQLError> {
    let mut inner   = pair.into_inner();
    let name        = inner.next().unwrap().as_str().to_string();
...
fn parse_subscription(pair: Pair<Rule>) -> Result<SubscriptionDef, AgentQLError> {
    let mut inner = pair.into_inner();
    let name      = inner.next().unwrap().as_str().to_string();
    let (variables, directives, pipelines) = parse_operation_body(inner)?;
...
) -> Result<(Vec<VariableDef>, Vec<Directive>, Vec<Pipeline>), AgentQLError> {
...
fn parse_variable_def(pair: Pair<Rule>) -> Result<VariableDef, AgentQLError> {
    let mut inner = pair.into_inner();
...
fn parse_pipeline(pair: Pair<Rule>) -> Result<Pipeline, AgentQLError> {
    let mut steps = vec![];
...
fn parse_field_call(pair: Pair<Rule>) -> Result<FieldCall, AgentQLError> {
    let mut module     = None;
...
fn parse_named_arg(pair: Pair<Rule>) -> Result<(String, Value), AgentQLError> {
    let mut inner = pair.into_inner();
...
fn parse_selection_set(pair: Pair<Rule>) -> Result<SelectionSet, AgentQLError> {
    let fields = pair
        .into_inner()
        .map(parse_selected_field)
        .collect::<Result<_, _>>()?;
    Ok(SelectionSet { fields })
}

fn parse_selected_field(pair: Pair<Rule>) -> Result<SelectedField, AgentQLError> {
    match pair.as_rule() {
        Rule::spread_field => {
        ...
        Rule::state_field => {
            let selectors = pair
                ...
                .map(|p| match p.as_str() {
                    ...
                    s => Err(AgentQLError::ParseError(format!("unknown state selector: {s}"))),
                })
                .collect::<Result<_, _>>()?;
...
fn parse_value(pair: Pair<Rule>) -> Result<Value, AgentQLError> {
    let inner = pair.into_inner().next().unwrap();
    match inner.as_rule() {
...
fn parse_directive(pair: Pair<Rule>) -> Result<Directive, AgentQLError> {
    let mut inner = pair.into_inner();
    let name      = inner.next().unwrap().as_str().to_string();
    let args      = inner
