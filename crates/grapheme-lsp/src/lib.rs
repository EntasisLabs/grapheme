//! Grapheme language server library.
//!
...
impl LanguageServer for Backend {
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        if let Some((namespace, type_name)) =
            namespaced_type_at_position(line, position.character as usize)
        {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        for spec in op_specs() {
            ...
            items.push(CompletionItem {
                ...
                detail: Some(format!("returns {}", signature_return_shape(spec))),
        ...
        if let Some((namespace, type_prefix)) =
            namespace_type_prefix_at_position(line, position.character as usize)
        {
    ...
    async fn goto_definition(
        ...
        for doc_uri in ordered_uris {
            ...
            if let Some(def) = index_definitions(doc_text)
                .into_iter()
        ...
        for (doc_uri, doc_text) in docs_snapshot {
            for range in symbol_occurrences(&doc_text, &symbol) {
                if !include_declaration
                    ...
                    && declaration_range
                        .as_ref()
                        .is_some_and(|decl| ranges_equal(decl, &range))
                {
                    continue;
                }
    ...
    async fn signature_help(&self, params: SignatureHelpParams) -> Result<Option<SignatureHelp>> {
        ...
        let Some((module, op, active_parameter)) =
            active_transform_call_at(line, position.character as usize)
        else {
            if let Some((target, active_parameter)) =
                active_user_call_at(line, position.character as usize)
            {
                let signatures = parse_executable_signatures(&text);
                if let Some(sig) = signatures.get(&target) {
                    let output = sig.output_type.as_deref().unwrap_or(&sig.input_type);
    ...
    async fn code_action(&self, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
        ...
        let (input_type, output_type) =
            enclosing_executable_signature(&text, line_number).unwrap_or(("Any".to_string(), None));
...
fn index_definitions(text: &str) -> Vec<DefinitionIndex> {
    ...
    for (idx, line) in text.lines().enumerate() {
        ...
        if let Some((name, start_char, end_char)) =
            parse_definition_name(trimmed, "subscription", line)
        {
            out.push(DefinitionIndex {
                name,
...
fn resolve_imported_type_fields(
    text: &str,
    uri: &Url,
    namespace: &str,
    type_name: &str,
...
fn enclosing_executable_signature(
    text: &str,
    line_number: usize,
...
fn parse_executable_signatures(text: &str) -> HashMap<String, ExecutableSignatureIndex> {
    ...
    for line in text.lines() {
        ...
        let Some((input_type, output_type)) = parse_executable_input_and_output_type(trimmed)
        else {
            continue;
        };
    ...
    while i < bytes.len() {
        match bytes[i] {
            ...
            _ if is_ident_char(bytes[i]) => {
                ...
                if is_keyword(ident) {
                    out.push(line_num, start as u32, (i - start) as u32, TOKEN_KEYWORD);
...
/// Run the Grapheme LSP over explicit async IO streams.
pub async fn run_server<R, W>(stdin: R, stdout: W)
where
    R: AsyncRead + Unpin,
    W: AsyncWrite + Unpin,
{
...
/// Run the Grapheme LSP over stdio.
